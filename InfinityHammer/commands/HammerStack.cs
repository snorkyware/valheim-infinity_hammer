...
public class HammerStackCommand
{
  ...
  private static List<string> AC(int index)
  {
    ...
    if (index == 1)
      return ParameterInfo.Create(
        "Step size (<color=yellow>number</color> or <color=yellow>number*auto</color> for automatic step size).");
    return ParameterInfo.None;
  }

  private static string Description(string direction) =>
    $"[amount] [step=auto] - Places multiple objects towards the {direction} direction.";
  ...
  public HammerStackCommand()
  {
    ...
    AutoComplete.Register("hammer_stack", (int index, int subIndex) =>
    {
      ...
      if (index == 1)
        return ParameterInfo.FRU(
          "Step size (<color=yellow>number</color> or <color=yellow>number*auto</color> for automatic step size)",
          subIndex);
      return ParameterInfo.None;
    });
    Helper.Command("hammer_stack",
      "[forward,up,right] [step=auto,auto,auto] - Places multiple objects next to each other.", (args) =>
      {
        HammerHelper.CheatCheck();
        Helper.ArgsCheck(args, 2, "Missing the amount.");
        Hammer.Equip();
  ...
  {
    var player = Helper.GetPlayer();
    if (player.m_placementGhost && player.m_placementGhost.TryGetComponent<Piece>(out var piece) &&
      !piece.m_repairPiece)
      return player.m_placementGhost;
    var hovered = Selector.GetHovered(player, Configuration.Range, Array.Empty<string>(), Configuration.IgnoredIds);
